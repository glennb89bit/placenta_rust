...
"#);
    ...
    // Check content of the options and return their final values as strings
    let (a, w, n) = check_variables(option_age.clone(), option_weight.clone(), option_number.clone());

    // Add addional white line if additional stdin was asked
    ...
    // Check the number of kids and refer to correct subprogram
    match number {
    ...
    // Start pause subprogram, so program doesn't close after finish
    pause();

}

fn check_variables(option_age:Option<String>, option_weight:Option<String>, option_number:Option<String> ) -> (String, String, String) {
    // Function checks if value present in parameter option, otherwise asks value in stdin
...
fn extra_line(line: bool) {
    // Print extra line in case 1 or more variables have been asked
    if line == true {
        println!();
    }
}
...
fn singleton(age: i32, weight: i32) {
    // Subprogram for singleton placentas (n=1)

    // Create dataframe from reference values
    let df = df!(
    ...
    // Filter dataframe on gestational age
    let filter = df
        .clone()
        .lazy()
    ...
    // Iterate over percentiles and match closes percentile for weight
...
fn twin(age: i32, weight: i32) {
    // Subprogram for twin placentas (n=1)
...
fn closest(weight: i32, weights: Vec<i32>) -> String {
    // Program that iterates and remembers the closest match
    ...
    // Print result based on the position of the closest match
    let result;
...
fn index_mapper (index: usize) -> String {
    // Function that maps the index of the vector to the correct percentile
...
fn closesttrip(weight: i32, weights: Vec<i32>) -> String {
    // Function to find closest value in a vector for triplets
    let mut absvalue: i32 = 1000;
    let mut value: i32 = 0;
    for  w in weights.iter().cloned() {
        let absolute = (w-weight).abs();
...
fn index_mapper_trip (index: usize) -> String {
    // Function to map index to percentiles for triplets
    let percentile: String = match index {
        0 => "P10".to_string(),
        1 => "P50".to_string(),
        2 => "P90".to_string(),
...
fn pause() {
    // Function to pause the program in the end
